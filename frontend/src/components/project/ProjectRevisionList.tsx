...
export default function ProjectRevisionList({ projectId }: { projectId: string }) {
  ...
  if (!revisions?.length) {
    return <div className="text-center py-8 text-gray-400 text-sm">No revisions yet</div>
  }
        ...
        <div key={rev.id} className="card">
          <div className="card-header">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-900">Revision {rev.revision_number}</p>
                  <p className="text-xs text-gray-500">By {rev.revised_by_name} · {fmtDateTime(rev.created_at)}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                {rev.departments_reopened?.length > 0 && (
                  <span className="badge-yellow">{rev.departments_reopened.length} depts reopened</span>
                )}
              </div>
            </div>
          </div>
          <div className="card-body space-y-3 text-sm">
            <div>
              ...
              <p className="text-gray-800">{rev.reason}</p>
            </div>
            {rev.client_request && (
              <div>
                <p className="text-xs text-gray-500 mb-1">Client Request</p>
                <p className="text-gray-700">{rev.client_request}</p>
              </div>
            )}
